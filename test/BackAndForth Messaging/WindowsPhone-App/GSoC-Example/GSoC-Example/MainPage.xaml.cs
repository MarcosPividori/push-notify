...
namespace GSoC_Example
{
    public partial class MainPage : PhoneApplicationPage
    {
        ...
        private String user = "", password = "" , uri = "", message = "";
        ...
        public MainPage()
        {
            ...
            string channelName = "MessagingPushChannel";
        ...
        Boolean register()
        {
            ...
            for (int i = 1; i <= MAX_ATTEMPTS; i++) {
                try {
                    UpdateConsole(String.Format("Trying (attempt {0}/{1}) to register device on Yesod Server.",
                    i, MAX_ATTEMPTS));
                    if (SendUserInfo())
                    {
        ...
        {
            Uri url = new Uri("http://gsoc.marcospividori.com.ar/fromdevices/register");
        ...
        void GetRequestStreamCallback(IAsyncResult asynchronousResult)
        {
            try
            {
                ...
                map.Add("regId", uri);
                map.Add("system", "WPHONE");

                String body = dictionaryToJson(map);
        ...
        void GetRequestStreamCallbackMessage(IAsyncResult asynchronousResult)
        {
            try
            {
                RequestState myRequestState = (RequestState)asynchronousResult.AsyncState;
            ...
            if (MessageText.Text.Length > 0)
            {
                ...
                Thread t = new Thread((ThreadStart)delegate
                {
                    if (SendMessageToServer())
                        UpdateConsole(String.Format("Sent [{0}]: {1}", DateTime.Now.ToShortTimeString(), message));
                });
                t.Start();
            }
            MessageText.Text = "";
